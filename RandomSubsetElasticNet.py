...
from RecursiveBooleanPhrase import RecursiveBooleanPhrase
from ModelPhraseDataObject import ModelPhraseDataObject
            ...
            for i in range(0, len(feature_set)):
                if i in list(self.feature_indices_to_values.keys()) and feature_set[i] not in \
                    ...
                    if len(self.feature_indices_to_values[i]) > 2:
                        ...
                        raise ValueError("Non-binary feature detected during training at index %s. Values found: %s", i,
                                         str(self.feature_indices_to_values[i]))
